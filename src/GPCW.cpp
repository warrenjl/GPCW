...
for(int j = 1; j < mcmc_samples; ++j){
  ...
  if((j % 100) == 0){
    double completion = j/(double)mcmc_samples;
    Rcpp::Rcout << completion << std::endl;
    double accrate_phi1_trans = acctot_phi1_trans/j;
    Rcpp::Rcout << accrate_phi1_trans << std::endl;
